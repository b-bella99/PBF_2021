...
export default class Student extends Component {
    ...
    getStudents() {
        axios.get("http://localhost:8000/api/students").then((response) => {
            ...
            if (
                response.data.status === "failed" && response.data.success === false
            ) {
                this.setState({
    ...
    addStudent = () => {
        axios
            .post(
                "http://localhost:8000/api/create-student", this.state.newStudentData
            )
            .then((response) => {
                const { students } = this.state;
                const newStudents = [...students];
                newStudents.push(response.data); this.setState(
                    {
                        students: newStudents, newStudentModal: false, newStudentData: {
                            first_name: "",
                            last_name: "",
                            email: "",
                            phone: "",
    ...
    onChangeEditStudentHanler = (e) => {
        ...
        editStudentData[e.target.name] = e.target.value; this.setState({ editStudentData });
    };

    editStudent = (id, first_name, last_name, email, phone) => {
        this.setState({
            editStudentData: { id, first_name, last_name, email, phone }, editStudentModal: !this.state.editStudentModal,
        });
    };
    ...
    updateStudent = () => {
        ...
        } = this.state.editStudentData; this.setState({
            isLoading: true,
        });
        axios
            ...
            .then((response) => {
                this.getStudents(); this.setState({
                    editStudentModal: false,
                    editStudentData: { first_name, last_name, email, phone }, isLoading: false,
                });
            })
            .catch((error) => {
                    ...
                    <tr key={student.id}>
                        ...
                        <td>
                            <Button color="success" className="mr-3" size="sm"
                                onClick={() =>
                                    this.editStudent(student.id, student.first_name, student.last_name, student.email, student.phone
                                    )
                                }
                            >Edit</Button>
        ...
        return (
            <div className="App container mt-4" >
                ...
                <AddStudents
                    ...
                    newStudentModal={this.state.newStudentModal} onChangeAddStudentHandler={this.onChangeAddStudentHandler}
                    addStudent={this.addStudent}
                    newStudentData={newStudentData}
                />

                <EditStudent toggleEditStudentModal={this.toggleEditStudentModal} editStudentModal={this.state.editStudentModal} onChangeEditStudentHanler={this.onChangeEditStudentHanler} editStudent={this.editStudent} editStudentData={editStudentData} updateStudent={this.updateStudent}
                />
